modImp.grid.Logs = function (config) {
	config = config || {};
	if (!config.id) {
		config.id = 'modimp-grid-logs';
	}
	Ext.applyIf(config, {
	...
	modImp.grid.Logs.superclass.constructor.call(this, config);
...
Ext.extend(modImp.grid.Logs, MODx.grid.Grid, {
	windows: {},
	...
	getColumns: function (config) {
		return [{
			header: _('modimp_log_id'),
			dataIndex: 'id',
			sortable: true,
			width: 10
		}, {
			header: _('modimp_log_description'),
...
Ext.reg('modimp-grid-logs', modImp.grid.Logs);
